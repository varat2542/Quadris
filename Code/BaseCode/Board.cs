...
namespace Quadris {
  ...
  public class GridCellInfo {
    ...
    public class Board {
        ...
        public int ChangePalette(int colorin) {
            int colorout = colorin + 1;
            clearboard();
            return colorout;
        }
        ...
        public void checkforgamened() {
            for (int curRow = 0; curRow < Grid.GetLength(0); curRow++) {
                for (int col = 1; col < Grid.GetLength(1); col++) {
                    GridCellInfo cellInfo = Grid[curRow, col];
                    if (GetCellInfo(3, col)?.State == CellState.OCCUPIED_PREVIOUSLY) {
                        PauseGame();
                        clearboard();
                    }
                }
          ...
          for (int col = 0; col < Grid.GetLength(1); col++) {
            for (int dropRow = curRow; dropRow > 0; dropRow--) {
              Grid[dropRow, col] = Grid[dropRow - 1, col];
               System.Media.SoundPlayer player = new System.Media.SoundPlayer(@"C:\Users\zacha\OneDrive\Desktop\CSC-403\Quadris-main\Code\Quadris\Data\se.wav");
               player.Play();
            }
          }
          curRow--;
